...
Rollout: footer is on 14 operational skills (all am-*, deploy-checklist, debugging, incident-response, book/video-knowledge-extraction, spec-writing). Skipped — no recurring operational facts: code-review, competitive-analysis, tdd, jbtd, improve-skill. **P1 shipped:** `decisions/` store + `decisions` skill (log/search/supersede) + OIAE mapping in `improve-skill`. **P2 shipped:** `prune` (stale+contradiction cleanup), selective `scripts/auto-retrieve.py` (≤3, never wholesale), `scripts/rotate-skill-runs.sh`; regression evals via `improve-skill`'s per-skill `evals.json`. See `docs/superpowers/specs/2026-06-18-harness-improvement-prd.md` and `USAGE.md`.
